...
# Read PRD.md and extract from line 250 onwards (1-based)
prd_path = os.path.join('memory', 'PRD.md')
with open(prd_path, 'r', encoding='utf-8') as f:
    lines = f.readlines()

start_line = 250 - 1
snippet = ''.join(lines[start_line:])

python_code = dsl_to_python_multi_event(snippet, all_event_fields)
print('--- GENERATED TEMPLATE ---')
print(python_code)

# Execute template
exec_globals = {'__file__': os.path.abspath('backend/server.py'), '__name__': '__dsl_test__'}
exec(compile(python_code, '<dsl_prd_snippet>', 'exec'), exec_globals)
